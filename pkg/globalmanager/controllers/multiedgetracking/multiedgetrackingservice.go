...
const (
	VideoAnalyticsJob        = "VideoAnalyticsWorker"
	FeatureExtractionService = "FeatureExtractionWorker"
	ObjectQueryJob           = "ObjectQueryWorker"
	reIDPort                 = 5000
	FEPort                   = 6000
)
...
func (mc *Controller) Run(stopCh <-chan struct{}) {
	workers := 1
	...
	if !cache.WaitForNamedCacheSync(Name, stopCh, mc.podStoreSynced, mc.serviceStoreSynced) {
...
func (mc *Controller) sync(key string) (bool, error) {
	...
	var neededPodCounts int32 = *sharedMultiEdgeTrackingService.Spec.FeatureExtractionWorker.Replicas
...
func (mc *Controller) updateStatus(service *sednav1.MultiEdgeTrackingService) error {
	client := mc.client.MultiEdgeTrackingServices(service.Namespace)
...
func (mc *Controller) createObjectQueryJob(service *sednav1.MultiEdgeTrackingService, activePods *int32, activeDeployments *int32, fused_layers bool, prev_IP string, prev_port int32) (port int32, ip string, err error) {
	...
	*/
	...
	sector, err := runtime.IdentifyNodeSector(mc.kubeClient, service.Spec.ObjectQueryWorker.Template.Spec.NodeName)
	fluentdIP, err := runtime.FindColocatedFluentdPod(mc.kubeClient, service.Spec.ObjectQueryWorker.Template.Spec.NodeName)
	...
	workerParam.WorkerType = ObjectQueryJob
	...
	if fused_layers {
		...
		feModelName := service.Spec.FeatureExtractionWorker.Model.Name
	...
	//_, err = runtime.CreateDeploymentWithTemplate(mc.kubeClient, service, &service.Spec.ObjectQueryWorker.Template, &workerParam, reIDPort)
	if err != nil {
	...
	*activeDeployments++
	...
	// reIDURL, err := runtime.CreateEdgeMeshService(mc.kubeClient, service, ObjectQueryWorker, reIDPort)
	reIDPortService, err := runtime.CreateKubernetesService(mc.kubeClient, service, ObjectQueryJob, reIDPort, reIDIP)
	if err != nil {
		return 0, "", fmt.Errorf("failed to create reid workers deployment: %w", err)
	}
...
func (c *Controller) createFeatureExtractionWorker(service *sednav1.MultiEdgeTrackingService, activePods *int32, activeDeployments *int32) (err error) {
	var workerParam runtime.WorkerParam
	var secretName string
	var modelSecret *v1.Secret
	...
	*/
	...
	workerParam.WorkerType = FeatureExtractionService
	...
	_, err = runtime.CreateDeploymentWithTemplate(c.kubeClient, service, &service.Spec.FeatureExtractionWorker.DeploymentSpec, &workerParam, FEPort)
	if err != nil {
...
func (mc *Controller) createVideoAnalyticsJob(service *sednav1.MultiEdgeTrackingService, activePods *int32, activeDeployments *int32, prev_IP string, prev_port int32, api_port int32, api_IP string) (err error) {
	var workerParam runtime.WorkerParam
	var secretName string
	var modelSecret *v1.Secret
	...
	*/
	...
	workerParam.WorkerType = VideoAnalyticsJob

	// Load model used by the pods in this deployment
	detectionModelName := service.Spec.VideoAnalyticsWorker.Model.Name
	...
	//_, err = runtime.CreateDeploymentWithTemplate(mc.kubeClient, service, &service.Spec.VideoAnalyticsWorker.Spec, &workerParam, 7000)
	if err != nil {
	...
	*activeDeployments++
	...
	_, err = runtime.CreateEdgeMeshService(mc.kubeClient, service, VideoAnalyticsJob, 7000)
	if err != nil {
		return fmt.Errorf("Failed to create edgemesh service: %w", err)
	}
...
func (mc *Controller) createWorkers(service *sednav1.MultiEdgeTrackingService) (activePods int32, activeDeployments int32, err error) {
	...
	klog.Infof("create feature extraction service: %s", service.Name)
	err = mc.createFeatureExtractionWorker(service, &activePods, &activeDeployments)
	if err != nil {
